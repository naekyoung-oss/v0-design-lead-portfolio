...
import { MONO, hoverRowSx, bodyTextSx } from "./styles"
...
export function Contact() {
  ...
  return (
    <Box component="section" id="contact" sx={{ marginTop: { xs: "59px", md: "67px" } }}>
      <SectionHeader index="— 05 —" title="Contact" />

      <Container maxWidth={false} disableGutters>
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1px 1fr" } }}>
          {/* Left: CTA */}
          <Box sx={{ px: { xs: 3, md: 6 }, py: { xs: 6, md: 8 } }}>
            <Typography
              variant="h4"
              sx={{ fontSize: { xs: "28px", md: "36px", lg: "44px" }, fontWeight: 900, letterSpacing: "-0.03em", lineHeight: 1.05, textTransform: "uppercase", color: "text.primary", mb: 3 }}
            >
              Let's discuss
              <br />
              a project.
            </Typography>
            <Typography variant="body1" sx={{ ...bodyTextSx, maxWidth: "48ch" }}>
              ...
              const inner = (
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: 2.5 }}>
                    ...
                    <Box>
                      <Typography sx={{ fontFamily: MONO, fontSize: "10px", letterSpacing: "0.14em", textTransform: "uppercase", color: "text.secondary", mb: 0.25 }}>
                        {item.label}
                      </Typography>
                  ...
                  {item.href && <ArrowForwardIcon sx={{ fontSize: 18, color: "primary.main" }} />}
                </Box>
              )

              if (item.href && item.external) {
                return (
                  <MuiLink key={item.label} href={item.href} target="_blank" rel="noopener noreferrer" underline="none" color="inherit">
                    {inner}
                  </MuiLink>
                )
              } else if (item.href) {
